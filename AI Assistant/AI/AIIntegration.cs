...
namespace AI_Assistant.AI
{
    ...
    {
        public const string ContextRouterBuild =
            "context-router-v2";
        ...
        private TaskDomain lastTaskDomain;

        private string lastDomainContext;

        private int requestSequence;

        private int lastDomainRequestSequence;

        private TaskDomain currentRequestDomain;

        private string pendingAmbiguousPrompt;
        ...
        private const int MaxEmptyResponseRecoveryAttempts =
            1;
        ...
        private const int MaxRoutingContextChars =
            5000;


        private const int MaxRoutingSegmentChars =
            1200;
        ...
        )
        {
            ...
            currentRequestDomain =
                TaskDomain.None;


            pendingAmbiguousPrompt =
                "";
        }
        ...
        )
        {
            ...
            string effectivePrompt =
                ResolvePendingTargetPrompt(
                    prompt,
                    out string? pendingClarification
                );
            ...
            if (
                actionableRequest
                && currentRequestDomain == TaskDomain.Blender
            )
            {
                return
                    AddDeterministicAssistantReply(
                        "Prepoznao sam da je zadatak za Blender, ali Blender bridge još nije povezan. Zadatak nije izvršen."
                    );
            }
            ...
            if (
                actionableRequest
                && currentRequestDomain == TaskDomain.SubstancePainter
            )
            {
                return
                    AddDeterministicAssistantReply(
                        "Prepoznao sam da je zadatak za Substance Painter, ali Substance Painter bridge još nije povezan. Zadatak nije izvršen."
                    );
            }
            ...
            if (
                actionableRequest
                && currentRequestDomain == TaskDomain.None
            ...
            )
            {
                ...
                ReportActivity(
                    "[CONTEXT] Nedostaje ciljna aplikacija; zadatak je sačuvan."
                );


                return
                    AddDeterministicAssistantReply(
                        GetTargetApplicationQuestion()
                    );
            }
            ...
            int emptyResponseRecoveryAttempts =
                0;
            ...
            )
            {
                ...
                )
                {
                    if (
                        ...
                        MaxEmptyResponseRecoveryAttempts
                    )
                    {
                        emptyResponseRecoveryAttempts++;
                        ...
                        ReportActivity(
                            "[MODEL RECOVERY] Prazan odgovor; tražim jednu završnu poruku ili ispravan tool poziv."
                        );
                        ...
                        baseMessages.Add(
                            new
                            {
                                ...
                                content =
                                    "Your previous assistant message had empty or null content. " +
                                    "Continue the current user request now. " +
                                    "If a matching tool is available and a real change is required, call it. " +
                                    "If tools already ran, inspect their results and return a concise non-empty final answer. " +
                                    "Do not repeat a successful mutation tool merely to produce text."
                            }
                        );
                    ...
                    answer =
                        "Model je dva puta vratio prazan odgovor. Zadatak nije potvrđen kao završen; pokušaj ponovo s kratkim opisom šta tačno ne radi.";
                }
...
""""
        ...
        )
        {
            ...
            bool contextualFollowUp =
                IsContextualFollowUp(
                    currentText
                );


            TaskDomain explicitDomain =
                ClassifyExplicitDomain(
                    currentText
                );


            bool inheritPreviousDomain =
                explicitDomain == TaskDomain.None
                && contextualFollowUp
                && lastTaskDomain != TaskDomain.None
                && requestSequence - lastDomainRequestSequence <=
                    MaxDomainContextAge;


            TaskDomain effectiveDomain =
                explicitDomain != TaskDomain.None
                    ? explicitDomain
                    : inheritPreviousDomain
                        ? lastTaskDomain
                        : TaskDomain.None;
            ...
            string text =
                BuildToolRoutingText(
                    currentText,
                    inheritPreviousDomain
                );


            if (explicitDomain != TaskDomain.None)
            {
                RememberTaskDomain(
                    explicitDomain,
                    currentText
        ...
        private string BuildToolRoutingText(
            string currentText,
            bool inheritPreviousDomain
        )
        {
            if (!inheritPreviousDomain)
            {
                return currentText;
            }
        ...
        private static bool IsContextualFollowUp(
            string text
        )
        {
            if (text.Length > 320)
        ...
        private static TaskDomain ClassifyExplicitDomain(
            string text
        )
        {
            if (
            ...
            {
                return TaskDomain.SelfDevelopment;
            }
            ...
            {
                return TaskDomain.SubstancePainter;
            }
            ...
            {
                return TaskDomain.Filesystem;
            }

            return TaskDomain.None;
        }


        private string ResolvePendingTargetPrompt(
            string prompt,
            out string? clarificationResponse
        )
        {
            ...
            )
            {
                ...
                ReportActivity(
                    "[CONTEXT] Sačuvani zadatak je otkazan."
                );
            ...
            if (
                selectedDomain == TaskDomain.Unity
                || selectedDomain == TaskDomain.Blender
                || selectedDomain == TaskDomain.SubstancePainter
            )
            {
                string savedPrompt =
                    pendingAmbiguousPrompt;
                ...
                ReportActivity(
                    "[CONTEXT] Sačuvani zadatak nastavljen u "
        ...
        private string AddDeterministicAssistantReply(
            string answer
        )
        {
            conversationHistory.Add(
        ...
        private static string GetTargetApplicationQuestion()
        {
            return
                "Gdje želiš da napravim — Unity, Blender ili Substance Painter? Zadatak sam sačuvao, pa možeš odgovoriti samo imenom aplikacije.";
        }


        private static string GetDomainDisplayName(
            TaskDomain domain
        )
        {
            return
        ...
        private static bool IsActionableRequest(
            string text
        )
        {
            return
        ...
        private void RememberTaskDomain(
            TaskDomain domain,
            string context
        )
        {
        ...
        private static string NormalizeRoutingText(
            string value
        )
        {
            string decomposed =
        ...
        private static string TrimRoutingSegment(
            string value
        )
        {
            if (value.Length <= MaxRoutingSegmentChars)
            {
                return value;
            }
            ...
            return
                value.Substring(
                    value.Length - MaxRoutingSegmentChars
                );
        }


        private static string TrimRoutingContext(
            string value
        )
        {
            if (value.Length <= MaxRoutingContextChars)
            {
                return value;
            }
            ...
            return
                value.Substring(
                    value.Length - MaxRoutingContextChars
                );
        }
